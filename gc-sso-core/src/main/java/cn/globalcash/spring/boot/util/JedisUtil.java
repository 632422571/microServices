package cn.globalcash.spring.boot.util;
...
public class JedisUtil {
    ...
    /**
     * redis address, like "{ip}"、"{ip}:{port}"、"{redis/rediss}://gc-sso:{password}@{ip}:{port:6379}/{db}"；Multiple "," separated
     */
    ...
    private static ShardedJedis getInstance () {
                ...
                if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)) {
                    try {
                        if (shardedJedisPool == null) {
                            ...
                            logger.info(">>>>>>>>>>> gc-sso, JedisUtil.ShardedJedisPool init success.");
                        }
                    } finally {
        ...
        if (shardedJedisPool == null) {
            throw new NullPointerException(">>>>>>>>>>> gc-sso, JedisUtil.ShardedJedisPool is null.");
        }
    ...
    public static void main(String[] args) {
        String gcSsoRedisAddress = "redis://gc-sso:password@127.0.0.1:6379/0";
        gcSsoRedisAddress = "redis://127.0.0.1:6379/0";
        init(gcSsoRedisAddress);
